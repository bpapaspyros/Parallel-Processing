...
void InitVisualizationMode(int *argc, char *argv[], void (*AdvanceFrame)(), int *numCells, Cell **cells, int **cnumPars)
{
  ...
  glutCreateWindow("Visualization Mode");
...
#define USE_track_velocity
		...
		float	y = cell->p[j % PARTICLES_PER_CELL].y;
		y_Tot += y - domainMin.y;
  ...
  vMax.x = std::max(vMax.x * 0.99f, vMin.x + epsilon);
  vMax.y = std::max(vMax.y * 0.99f, vMin.y + epsilon);
  vMax.z = std::max(vMax.z * 0.99f, vMin.z + epsilon);
#else
  prior_y_Max = vMax.y;
  prior_y_Ave = y_Tot / (float)*_numCells;
#endif
  glEnd();
